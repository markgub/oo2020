...
public class Car{
    ...
    public void parkCar(ParkingPlace place, CarsInPark cars){
        if(inFight){
            ...
            for(int i = 0; i < cars.size(); i++){
                if(cars.get(i).place == place) {
                    occupied = true;
                    if(cars.get(i).mark == AutomobileMark.BMW && mark == AutomobileMark.BMW) {
                        inFight = true;
                        cars.get(i).inFight = true;
                    }
                    break;
                }
            }
